...
Rust里面存在指针，引用，解引用等等操作，被很多人认为与C或者C++很像，经常会放在一起比较。

虽然他们都是对于堆栈的数据操作，但如果从rust的生命周期理解，我会觉得Rust的指针的目的，更多的是显示的管理用户所有权和生命周期。而在Java/Python一类的语言中，堆栈的操作被弱化了，是由GC帮忙管理的。

我不觉得Rust是个更好或更坏的 [C/C++]++。他们是不同的。

从Java，Python，Go的等高级语言迁移过来的开发者，会看到很多熟悉的影子。

那么Rust是什么？

在Rust的官网上，写了Rust的三大特性，我们逐一解读一下。
